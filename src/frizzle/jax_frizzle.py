...
if __name__ == "__main__":
    ...
    def prepare_dataset(xs, ys, ivars, bs, delta_xs):
        xs = jnp.hstack([xs - dx for dx in delta_xs])
        ys = jnp.hstack(ys)
        ivars = jnp.hstack(ivars)
        mask = ~jnp.hstack(bs).astype(bool)
        return dict(λ=xs, flux=ys, ivar=ivars, mask=mask)

    os_data = prepare_dataset(xs_os, ys_os, ivars_os, bs_os, delta_xs_os)

    xstar = jnp.array(xstar)
    ystar, ivar, flags_os, meta_os = frizzle(xstar, **os_data)
    ...
    for ax in axes:
        ...
        residual = ystar - true_spectrum(xstar, 0., *line_args_os)
