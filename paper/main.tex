...
ANDY: Should we mention something about if it is not telluric corrected, and you have many epochs of a source that has perceived radial velocity variations, then you want something like wobble?
...
Now for the method:
% ANDY: this used to say "We stack" and it triggered me because I just read 7 pages about how we shouldn't "stack"!
...
\end{align}
which is the maximum-likelihood or minimum-$\chi^2$ solution for a linear weighted least squares problem or a linear model with Gaussian noise (see, for example, \cite{hoggfitting}).
...
ANDY: continuum-rectified (unit-less) ?
