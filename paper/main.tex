...
% To-Do items
% -----------
% - What's our name? Choose and propagate into where "Forward Model(tm)" is used.
% - Put in notes about our hypothesis for WHY we get uncorrelated pixels.
% - Make comments directly about Drizzle to justify the name.
% - discuss conservation of flux vs intensity.
...
Minimum-variance unbiased estimators require care in their construction; they are generally maximum-likelihood estimators (\citealt{mvue}). % HOGG
...
More on this below. %% HOGG DO WE COME BACK TO THIS?

% HOGG SAY: IT is not at all clear here that we are NOT assuming critical sampling. We aren't! Make that clear.
% In general this section has issues.
...
We interpolate the model to account for the shifts, and we combine data to average away this noise. % ARC to HOGG: is 'average away' what you want to say here?
...
We return to these issues at the end. %% HOGG DO WE?
...
This is particularly evident in the second panel of Figure~\ref{fig:standard} near $\ln\lambda = 8.7024$.
...
All code used for this project is available at \url{https://github.com/davidwhogg/NoDataInterpolation}.% ANDY to update terminology in code. and is registered in the Python Package Index as \texttt{NoDataInterpolation}.
