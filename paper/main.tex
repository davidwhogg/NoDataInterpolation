...
It is wrong because minimum-variance unbiased estimators are (often) maximum-likelihood estimators (\citealt{something}).
The deep, high signal-to-noise image or spectrum we seek is a statistical estimate of the image or spectrum, given noisy data.
Noisy data are combined to make a measurement by building and optimizing a likelihood, not by shifting and stacking!
...
Our view is that image combination should be seen as a problem in statistical estimation.
Let's find the best estimator for the combined image we can find, and let's deliver that (along with an uncertainty estimate).
...
As we will show, an estimator also yields a more faithful representation\footnote{
    An estimator needs a \emph{model} to produce a combined spectrum, but we use the word \emph{representation} to avoid overloading the word `model', and to indicate that we will not construct a physical model of a spectrum from atomic physics.
    We only need a flexible representation!
}
of the underlying \emph{true} spectrum, critically important for any subsequent precision measurements (e.g., radial velocities, chemical abundances).
...
At the end we may (do we?) return to the question of how those shift operators are determined. ANDY: SMASH! Out of scope!
...
\paragraph{Bit-flag masks (or is it bit-masks? bit-pixel masks?)}
A bit-flag mask is often used to encode meta data about an observation that otherwise cannot be represented by the spectrum $y_i$ or its asssociated variance tensor $C_i$ (e.g., poorly described line spread function), and we often want to propagate this meta data to appropriate pixels in the combined spectrum.
These bit-flag masks are usually encoded as integers with up to $p$ potential flags, allowing for multiple flags to be encoded as a single value $\sum^{P}_{p=0} 2^p$. 
This bit-flag mask can be separated into $p$ masks where each has value 1 in the locations where the flag is marked, and 0 in the locations where it is not.
