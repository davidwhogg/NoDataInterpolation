...
After sampling the mean spectral expectation model onto an observational pixel grid, we add Gaussian noise with a uniform variance (HOGG NO WE DON'T!) such that the signal-to-noise in the continuum is XXXX per pixel in each epoch spectrum.
At a rate of XXX pixels were randomly and independently marked as ``bad'' and offset by a large offset.

HOGG: MAKE 2 DATA SETS, one poorly sampled, one well sampled.
...
\begin{figure}[t!]
    ...
    \end{center}
    \caption{The top 8 panels show the $N=8$ epochs of the poorly sampled input data. The bad pixels are indicated with grey bars. The bottom 2 panels show a zoom in on one of the epochs, along with the true spectrum used to generate the data. The true spectrum is shown at zero Doppler shift, while each epoch spectrum is at a finite Doppler shift $\Delta x_i$.\label{fig:data1}}
    \end{mdframed}
\end{figure}
\begin{figure}[t!]
    \begin{mdframed}\begin{center}
    ...
    \end{center}
    \caption{Similar to the top panels of \figurename~\ref{fig:data1} except showing the well sampled input data.\label{fig:data2}}
    \end{mdframed}
\end{figure}

For the combined spectrum we choose a pixel grid $x_\star$ with $M_\star$ pixels evenly spaced in log wavelength $x$, separated by $1/R$, which (just barely) samples the pixel-convolved LSF well.
...
\begin{figure}[t!]
    ...
    \end{center}
    \caption{HOGG: WRITE CAPTION.\label{fig:interpolated}}
    \end{mdframed}
\end{figure}
...
We find HOGG WHAT?? CORRELATIONS; WORSE RESIDUALS NEAR SPECTRAL FEATURES (UH OH).
\begin{figure}[t!]
    \begin{mdframed}\begin{center}
